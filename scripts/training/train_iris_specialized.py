...
def custom_collate_fn(batch):
    """IRIS-optimized collate function with color focus"""
    inputs = []
    outputs = []
    ...
    for item in batch:
        ...
        # Pad to target size for consistent color processing
        pad_h = target_size - H
        pad_w = target_size - W
...
def train_iris_specialized():
    ...
    for stage in range(IRIS_CONFIG['curriculum_stages']):
        ...
        for epoch in range(IRIS_CONFIG['epochs_per_stage']):
            ...
            if exact_dataset and epoch < 40:  # First 40 epochs for color learning
