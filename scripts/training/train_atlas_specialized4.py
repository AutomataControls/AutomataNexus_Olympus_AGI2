...
class Advanced2DSpatialDataset(Dataset):
        ...
        for file in data_files:
            ...
            if os.path.exists(file_path):
                with open(file_path, 'r') as f:
                    data = json.load(f)
                    for task in data:
                        self._process_advanced_spatial_task(task, file)
    ...
    def _create_advanced_spatial_sample(self, example: Dict, is_arc_task: bool) -> Optional[Dict]:
        ...
        # Filter for advanced spatial relevance
        if self.spatial_focus and spatial_analysis['spatial_intelligence_level'] < 3:
            if random.random() > 0.25:  # Keep 25% of simple cases
                return None
...
def train_advanced_spatial_stage(model, dataloader, criterion, optimizer, scheduler, scaler,
    ...
    for epoch in range(epochs_for_stage):
        ...
        # Progress bar
        pbar = tqdm(dataloader, desc=f"\033[96mAdvanced Spatial Stage {stage_idx} Epoch {epoch}\033[0m")
        ...
        if epoch % 4 == 0 or epoch == epochs_for_stage - 1:
            ...
            print(f"\033[96mAdvanced Spatial Stage {stage_idx} Epoch {epoch}: "
