...
class ExtendedCreativeDataset(Dataset):
    ...
    def _load_extended_creative_data(self):
        """Load data with extended creative complexity focus and ARC specificity"""
        data_files = [
        ...
        arc_emphasis = 2 if self.arc_specific else 1
            ...
            if os.path.exists(file_path):
                with open(file_path, 'r') as f:
                    ...
                    for _ in range(emphasis_count):
                        for task in data:
                            self._process_extended_creative_task(task, file)
    ...
    def _create_extended_creative_sample(self, example: Dict, is_arc_task: bool) -> Optional[Dict]:
        ...
        # Filter for extended creative relevance (more lenient for V5)
        if self.creative_focus and creative_analysis['creative_intelligence_level'] < 1:
            if random.random() > 0.5:  # Keep 50% of simple cases for V5
                return None
